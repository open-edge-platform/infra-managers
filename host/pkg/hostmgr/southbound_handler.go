...
func updateHostdevice(ctx context.Context, tenantID string, hostRes *computev1.HostResource, deviceInfo *pb.DeviceInfo) error {
	...
	zlog.Debug().Msgf("device name. inventory=%s, deviceInfo=%s, hostDevice=%s",
		invDevice.GetDeviceName(), deviceInfo.GetHostname(), hostDevice.GetDeviceName())
	...
	if exists {
		...
		devicenameReceived := hostDevice.GetDeviceName() == ""
		if !devicenameReceived {
			// New device info to be added to the inventory
			err = hostDeviceToAddOrUpdate(ctx, tenantID, devicenameReceived, hostDevice, invDevice)
			if err != nil {
				return err
			}
		} else {
