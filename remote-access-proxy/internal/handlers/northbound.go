...
func NewNBHandler(netCl *clients.RmtAccessInventoryClient,
	...
	rmtAccessConfReconciler, err := reconcilers.NewRAPReconciler(netCl, nil, tracingEnabled)
	if err != nil {
		return nil, err
	}
	rmtAccessConfController := rec_v2.NewController[reconcilers.ReconcilerID](rmtAccessConfReconciler.Reconcile, rec_v2.WithParallelism(parallelism))
	controllers[inv_v1.ResourceKind_RESOURCE_KIND_RMT_ACCESS_CONF] = rmtAccessConfController
...
func (nbh *NBHandler) filterEvent(event *inv_v1.SubscribeEventsResponse) bool {
	...
	if !ok {
		// No filter registered for this kind => event is ignored.
		return false
	}
