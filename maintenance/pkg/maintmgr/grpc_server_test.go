...
//nolint:funlen // long function due to comprehensive CVE testing
func TestServer_PlatformUpdateStatus_CVEHandling(t *testing.T) {
	dao := inv_testing.NewInvResourceDAOOrFail(t)
	ctx, cancel := inv_testing.CreateContextWithENJWT(t, mm_testing.Tenant1)
	defer cancel()

	// Create immutable OS with CVE data
	immutableOSProfileName := "immutable-cve-test-profile"
	osImageSha256 := inv_testing.GenerateRandomSha256()
	existingCVEsData := `[{"id":"CVE-2023-1234","severity":"high"},{"id":"CVE-2023-5678","severity":"medium"}]`
	fixedCVEsData := `[{"id":"CVE-2022-9999","severity":"critical"}]`

	immutableOSWithCVEs := dao.CreateOsWithOpts(t, mm_testing.Tenant1, true, func(os *os_v1.OperatingSystemResource) {
		os.Name = "Immutable OS with CVEs"
		os.Sha256 = osImageSha256
		os.ProfileName = immutableOSProfileName
		os.ProfileVersion = "2.0.0"
		os.SecurityFeature = os_v1.SecurityFeature_SECURITY_FEATURE_NONE
		os.OsType = os_v1.OsType_OS_TYPE_IMMUTABLE
		os.ExistingCves = existingCVEsData
		os.FixedCves = fixedCVEsData
		os.ExistingCvesUrl = "/cves/existing.json"
		os.FixedCvesUrl = "/cves/fixed.json"
	})

	// Create mutable OS with CVE data (to ensure CVE copying doesn't happen for mutable)
	mutableOSWithCVEs := dao.CreateOsWithOpts(t, mm_testing.Tenant1, true, func(os *os_v1.OperatingSystemResource) {
		os.Name = "Mutable OS with CVEs"
		os.Sha256 = inv_testing.GenerateRandomSha256()
		os.ProfileName = "mutable-cve-test-profile"
		os.ProfileVersion = "1.5.0"
		os.SecurityFeature = os_v1.SecurityFeature_SECURITY_FEATURE_NONE
		os.OsType = os_v1.OsType_OS_TYPE_MUTABLE
		os.ExistingCves = existingCVEsData
		os.FixedCves = fixedCVEsData
		os.ExistingCvesUrl = "/cves/existing-mutable.json"
		os.FixedCvesUrl = "/cves/fixed-mutable.json"
	})

	// Create OS update policies
	immutableOSUpdatePolicy := dao.CreateOSUpdatePolicy(
		t, mm_testing.Tenant1,
		inv_testing.OsUpdatePolicyName("CVE Test Immutable OS Update Policy"),
		inv_testing.OSUpdatePolicyTarget(),
		inv_testing.OSUpdatePolicyTargetOS(immutableOSWithCVEs),
	)

	mutableOSUpdatePolicy := dao.CreateOSUpdatePolicy(
		t, mm_testing.Tenant1,
		inv_testing.OsUpdatePolicyName("CVE Test Mutable OS Update Policy"),
		inv_testing.OSUpdatePolicyTarget(),
		inv_testing.OSUpdatePolicyUpdateSources([]string{"http://repo.example.com"}),
		inv_testing.OSUpdatePolicyKernelCommand("console=ttyS0"),
		inv_testing.OSUpdatePolicyInstallPackages("curl wget"),
	)

	// Create hosts with instances
	// Host with immutable OS instance
	hostImmutable := mm_testing.HostResource1 //nolint:govet // ok to copy locks in test
	hostImmutable.Uuid = uuid.NewString()
	hostImmutableRes := mm_testing.CreateHost(t, mm_testing.Tenant1, &hostImmutable)

	dao.CreateInstanceWithOpts(t, mm_testing.Tenant1, hostImmutableRes, immutableOSWithCVEs, true,
		func(inst *computev1.InstanceResource) {
			inst.ProvisioningStatus = om_status.ProvisioningStatusDone.Status
			inst.ProvisioningStatusIndicator = om_status.ProvisioningStatusDone.StatusIndicator
			// Initially, the instance should not have CVE data - it will be copied during PlatformUpdateStatus
			inst.ExistingCves = ""
		},
		inv_testing.InstanceOsUpdatePolicy(immutableOSUpdatePolicy),
	)

	// Host with mutable OS instance
	hostMutable := mm_testing.HostResource1 //nolint:govet // ok to copy locks in test
	hostMutable.Uuid = uuid.NewString()
	hostMutableRes := mm_testing.CreateHost(t, mm_testing.Tenant1, &hostMutable)

	dao.CreateInstanceWithOpts(t, mm_testing.Tenant1, hostMutableRes, mutableOSWithCVEs, true,
		func(inst *computev1.InstanceResource) {
			inst.ProvisioningStatus = om_status.ProvisioningStatusDone.Status
			inst.ProvisioningStatusIndicator = om_status.ProvisioningStatusDone.StatusIndicator
			// For mutable OS, instance CVE data should remain unchanged
			inst.ExistingCves = ""
		},
		inv_testing.InstanceOsUpdatePolicy(mutableOSUpdatePolicy),
	)
	...
	testCases := map[string]struct {
		hostGUID    string
		description string
		validate    func(t *testing.T, resp *pb.PlatformUpdateStatusResponse)
	}{
		"ImmutableOS_CVEDataCopiedFromOSToInstance": {
			hostGUID:    hostImmutableRes.Uuid,
			description: "For immutable OS, CVE data should be copied from OS resource to instance resource",
			validate: func(t *testing.T, resp *pb.PlatformUpdateStatusResponse) {
				// Verify response structure
				assert.NotNil(t, resp)
				assert.Equal(t, pb.PlatformUpdateStatusResponse_OSType(os_v1.OsType_OS_TYPE_IMMUTABLE), resp.OsType)

				// For immutable OS, verify the OS profile update source is populated correctly
				// This indicates that the CVE handling path for immutable OS was executed
				assert.NotNil(t, resp.OsProfileUpdateSource)
				assert.Equal(t, immutableOSProfileName, resp.OsProfileUpdateSource.ProfileName)
				assert.Equal(t, "2.0.0", resp.OsProfileUpdateSource.ProfileVersion)
				assert.Equal(t, osImageSha256, resp.OsProfileUpdateSource.OsImageSha)

				// Verify that update source fields are properly initialized for immutable OS
				assert.NotNil(t, resp.UpdateSource)
				assert.Empty(t, resp.UpdateSource.KernelCommand, "Immutable OS should not have kernel command from policy")
				assert.Empty(t, resp.UpdateSource.CustomRepos, "Immutable OS should not have custom repos from policy")
				assert.Empty(t, resp.InstalledPackages, "Immutable OS should not have installed packages from policy")
			},
		},
		"MutableOS_CVEDataNotCopied": {
			hostGUID:    hostMutableRes.Uuid,
			description: "For mutable OS, CVE data should NOT be copied from OS resource to instance resource",
			validate: func(t *testing.T, resp *pb.PlatformUpdateStatusResponse) {
				// Verify response structure
				assert.NotNil(t, resp)
				assert.Equal(t, pb.PlatformUpdateStatusResponse_OSType(os_v1.OsType_OS_TYPE_MUTABLE), resp.OsType)

				// Verify mutable OS specific fields are populated correctly from the OS update policy
				assert.NotNil(t, resp.UpdateSource)
				assert.Equal(t, "console=ttyS0", resp.UpdateSource.KernelCommand)
				assert.Equal(t, []string{"http://repo.example.com"}, resp.UpdateSource.CustomRepos)
				assert.Equal(t, "curl wget", resp.InstalledPackages)

				// For mutable OS, OS profile update source should be empty/default
				assert.NotNil(t, resp.OsProfileUpdateSource)
				// The mutable OS path doesn't populate OSProfileUpdateSource with target OS details
			},
		},
	}
	...
	for tcName, tc := range testCases {
		t.Run(tcName, func(t *testing.T) {
			...
			resp, err := MaintManagerTestClient.PlatformUpdateStatus(ctx, &pb.PlatformUpdateStatusRequest{
				HostGuid: tc.hostGUID,
				UpdateStatus: &pb.UpdateStatus{
					StatusType: pb.UpdateStatus_STATUS_TYPE_UP_TO_DATE,
				},
			})

			require.NoError(t, err, "PlatformUpdateStatus should succeed")
			tc.validate(t, resp)
		})
	}
}

func TestServer_PlatformUpdateStatus_CVEHandling_EdgeCases(t *testing.T) {
	dao := inv_testing.NewInvResourceDAOOrFail(t)
	ctx, cancel := inv_testing.CreateContextWithENJWT(t, mm_testing.Tenant1)
	defer cancel()

	// Test case 1: Immutable OS with empty CVE data
	immutableOSNoCVEs := dao.CreateOsWithOpts(t, mm_testing.Tenant1, true, func(os *os_v1.OperatingSystemResource) {
		os.Name = "Immutable OS No CVEs"
		os.Sha256 = inv_testing.GenerateRandomSha256()
		os.ProfileName = "immutable-no-cves-profile"
		os.ProfileVersion = "1.0.0"
		os.SecurityFeature = os_v1.SecurityFeature_SECURITY_FEATURE_NONE
		os.OsType = os_v1.OsType_OS_TYPE_IMMUTABLE
		// No CVE data set - testing empty string handling
		os.ExistingCves = ""
		os.FixedCves = ""
	})

	immutableOSUpdatePolicyNoCVEs := dao.CreateOSUpdatePolicy(
		t, mm_testing.Tenant1,
		inv_testing.OsUpdatePolicyName("No CVEs Immutable OS Update Policy"),
		inv_testing.OSUpdatePolicyTarget(),
		inv_testing.OSUpdatePolicyTargetOS(immutableOSNoCVEs),
	)

	// Host with immutable OS instance (no CVEs)
	hostNoCVEs := mm_testing.HostResource1 //nolint:govet // ok to copy locks in test
	hostNoCVEs.Uuid = uuid.NewString()
	hostNoCVEsRes := mm_testing.CreateHost(t, mm_testing.Tenant1, &hostNoCVEs)

	dao.CreateInstanceWithOpts(t, mm_testing.Tenant1, hostNoCVEsRes, immutableOSNoCVEs, true,
		func(inst *computev1.InstanceResource) {
			inst.ProvisioningStatus = om_status.ProvisioningStatusDone.Status
			inst.ProvisioningStatusIndicator = om_status.ProvisioningStatusDone.StatusIndicator
			inst.ExistingCves = ""
		},
		inv_testing.InstanceOsUpdatePolicy(immutableOSUpdatePolicyNoCVEs),
	)

	t.Run("ImmutableOS_EmptyCVEData", func(t *testing.T) {
		resp, err := MaintManagerTestClient.PlatformUpdateStatus(ctx, &pb.PlatformUpdateStatusRequest{
			HostGuid: hostNoCVEsRes.Uuid,
			UpdateStatus: &pb.UpdateStatus{
				StatusType: pb.UpdateStatus_STATUS_TYPE_UP_TO_DATE,
			},
		})

		require.NoError(t, err, "PlatformUpdateStatus should succeed even with empty CVE data")
		assert.NotNil(t, resp)
		assert.Equal(t, pb.PlatformUpdateStatusResponse_OSType(os_v1.OsType_OS_TYPE_IMMUTABLE), resp.OsType)

		// Verify that the CVE handling path for immutable OS was executed
		assert.NotNil(t, resp.OsProfileUpdateSource)
		assert.Equal(t, "immutable-no-cves-profile", resp.OsProfileUpdateSource.ProfileName)
		assert.Equal(t, "1.0.0", resp.OsProfileUpdateSource.ProfileVersion)
	})

	// Test case 2: Immutable OS with malformed CVE JSON (edge case)
	immutableOSMalformedCVEs := dao.CreateOsWithOpts(t, mm_testing.Tenant1, true, func(os *os_v1.OperatingSystemResource) {
		os.Name = "Immutable OS Malformed CVEs"
		os.Sha256 = inv_testing.GenerateRandomSha256()
		os.ProfileName = "immutable-malformed-cves-profile"
		os.ProfileVersion = "1.0.0"
		os.SecurityFeature = os_v1.SecurityFeature_SECURITY_FEATURE_NONE
		os.OsType = os_v1.OsType_OS_TYPE_IMMUTABLE
		// Malformed JSON to test robustness
		os.ExistingCves = `{"invalid": "json", "missing": "array_format"}`
		os.FixedCves = `["CVE-2023-0001"]`
	})

	immutableOSUpdatePolicyMalformed := dao.CreateOSUpdatePolicy(
		t, mm_testing.Tenant1,
		inv_testing.OsUpdatePolicyName("Malformed CVEs Immutable OS Update Policy"),
		inv_testing.OSUpdatePolicyTarget(),
		inv_testing.OSUpdatePolicyTargetOS(immutableOSMalformedCVEs),
	)

	hostMalformed := mm_testing.HostResource1 //nolint:govet // ok to copy locks in test
	hostMalformed.Uuid = uuid.NewString()
	hostMalformedRes := mm_testing.CreateHost(t, mm_testing.Tenant1, &hostMalformed)

	dao.CreateInstanceWithOpts(t, mm_testing.Tenant1, hostMalformedRes, immutableOSMalformedCVEs, true,
		func(inst *computev1.InstanceResource) {
			inst.ProvisioningStatus = om_status.ProvisioningStatusDone.Status
			inst.ProvisioningStatusIndicator = om_status.ProvisioningStatusDone.StatusIndicator
			inst.ExistingCves = ""
		},
		inv_testing.InstanceOsUpdatePolicy(immutableOSUpdatePolicyMalformed),
	)

	t.Run("ImmutableOS_MalformedCVEData", func(t *testing.T) {
		resp, err := MaintManagerTestClient.PlatformUpdateStatus(ctx, &pb.PlatformUpdateStatusRequest{
			HostGuid: hostMalformedRes.Uuid,
			UpdateStatus: &pb.UpdateStatus{
				StatusType: pb.UpdateStatus_STATUS_TYPE_UP_TO_DATE,
			},
		})

		require.NoError(t, err, "PlatformUpdateStatus should succeed even with malformed CVE data")
		assert.NotNil(t, resp)
		assert.Equal(t, pb.PlatformUpdateStatusResponse_OSType(os_v1.OsType_OS_TYPE_IMMUTABLE), resp.OsType)

		// Verify that the CVE handling path for immutable OS was executed correctly
		assert.NotNil(t, resp.OsProfileUpdateSource)
		assert.Equal(t, "immutable-malformed-cves-profile", resp.OsProfileUpdateSource.ProfileName)
		assert.Equal(t, "1.0.0", resp.OsProfileUpdateSource.ProfileVersion)
	})
}
